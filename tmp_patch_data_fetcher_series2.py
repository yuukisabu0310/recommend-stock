...
if 'long_term_rate_history' not in text:
    ...
    text = re.sub(r"(?m)^"+re.escape(ind)+r"long_term_rate\s*=\s*None\s*$",
                  ind+"long_term_rate = None\n"+ind+"long_term_rate_history = None  # logs繝√Ε繝ｼ繝育畑・育峩霑・縺区怦・・,
                  text, count=1)

    # Insert US FRED history after the FRED latest fetch line
    ...
    insert = fred_line + (
        "            # logs繝√Ε繝ｼ繝育畑・夂峩霑・縺区怦縺ｮ譎らｳｻ蛻暦ｼ・RED・噂n"
        "            if self.fred_client:\n"
        "                try:\n"
    ...
    # Insert yfinance 6mo history after the yfinance latest fetch warning line
    anchor = '                    logger.warning(f"髟ｷ譛滄≡蛻ｩ蜿門ｾ励お繝ｩ繝ｼ (yfinance, {country_code}): {e}")\n'
    if anchor in text:
        text = text.replace(anchor, anchor +
            "            # logs繝√Ε繝ｼ繝育畑・夂峩霑・縺区怦縺ｮ譎らｳｻ蛻暦ｼ・finance繝輔か繝ｼ繝ｫ繝舌ャ繧ｯ・噂n"
...
if 'CPI_history' not in text:
    # declare cpi_history after first cpi = None
    text, n = re.subn(r"(?m)^(\s*)cpi\s*=\s*None\s*$", r"\1cpi = None\n\1cpi_history = None  # logs繝√Ε繝ｼ繝育畑・亥燕蟷ｴ豈・縲∵怦谺｡・・, text, count=1)
    if n != 1:
        raise SystemExit('ERROR: could not declare cpi_history')

    # Insert YoY history calc after CPI險育ｮ励お繝ｩ繝ｼ log line (US block)
    ...
    text = text.replace(err_line, err_line +
        "\n                        # logs繝√Ε繝ｼ繝育畑・壼燕蟷ｴ豈斐・譎らｳｻ蛻暦ｼ域怙菴・2縺区怦・噂n"
        "                        try:\n"
