...
(function() {
    ...
    // 憲法準拠：changePeriod関数（Plotly.react()を使用）
    function changePeriod(chartId, years) {
        // Plotlyが読み込まれているか確認
        ...
        // 複数期間チャートデータが存在するか確認
        if (typeof window.multiPeriodChartData === 'undefined' || !window.multiPeriodChartData) {
            console.warn('Multi-period chart data is not available');
            return;
        }
        
        // チャートIDに応じて対応するチャートタイプを取得
        const chartTypeMap = {
            'price-chart': 'price',
            'rate-chart': 'rate',
            'cpi-chart': 'cpi',
            'eps-per-chart': 'eps_per'
        };
        
        const chartType = chartTypeMap[chartId];
        if (!chartType || !window.multiPeriodChartData[chartType]) {
            return;
        }
        
        // 指定期間のデータを取得
        const chartData = window.multiPeriodChartData[chartType];
        if (!chartData[years]) {
            console.warn('No data found for period:', years, 'years');
            return;
        }
        
        const periodData = chartData[years];
        if (!periodData.traces || !periodData.layout) {
            console.warn('Invalid period data structure');
            return;
        }
        ...
        // Plotly.react()で再描画（憲法準拠）
        Plotly.react(chartDiv, periodData.traces, periodData.layout, { responsive: true });
    }
    
    // 期間に応じてチャートを更新（後方互換性のため残す）
    function updateChartsForPeriod(years, chartId) {
        changePeriod(chartId, years);
    }
