...
本アプリは「**判断はルールベース**」「**表現はLLM（Groq）**」を厳格に分離します。

- **LLMは一切の市場判断・売買判断を行いません**
- LLMはルールベースで確定した状態・結論を投資家向けに要約・言語化する役割のみを担います

### 処理フロー

1. **生データ取得** → 2. **状態定義（enum）に変換** → 3. **ルールベースで市場判断** → 4. **LLMで要約・言語化**

## 機能

### 市場環境分析
- **Market Direction Overview**: 国別（米国・日本）× 期間（短期/中期/長期）ごとの市場方向感を5段階で表示
- **矢印UI**: 超強気〜弱気を色と矢印角度で視覚的に表現
- **状態ベース分析**: 生データを状態（enum）に変換し、ルールベースで判断
...
### セクターおすすめ
- **テーマベース推薦**: テーマ → 波及 → 周辺産業の流れで決定
- **主要テーマ**: 地政学リスク、AI集中、金融引き締め、インフレヘッジ、成長回復
...
- **Groq API**: 市場判断の要約・言語化（LLM: `llama-3.1-70b-versatile`）
  - **温度**: 0.2〜0.3（固定）
  - **最大トークン**: 300〜500（要約用）
...
- **市場判断はルールベースで決定**（LLMは要約・言語化のみ）
- **状態定義（enum）**: 生データを状態に変換し、人間が見て直感的に理解できる粒度で定義
