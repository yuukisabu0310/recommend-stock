...
{{
  "summary": "結論ブロック専用の2文要約（1文目：【結論】◯◯市場は（期間）で（方向ラベル）の形式、2文目：主要因を1つだけ短文で補足。各40-50文字以内。新しい論点は出さない、logsの内容を要約するだけ。断定表現禁止）",
...
重要：
- summaryは結論ブロック専用（ページ最上部に表示される2行固定の要約）
- summaryは必ず2文（1文目：【結論】◯◯市場は（期間）で（方向ラベル）の形式、2文目：主要因を1つだけ短文で補足）
- summaryの各文は40-50文字以内
- summaryには新しい論点を出さない（logsの内容を要約するだけ）
- summaryには新しいリスクの追加、未来時点の予測、「可能性が高い」などの強断定を禁止
